...
import { publicFeaturedCars } from "@/lib/fleet";
...
export default async function ServiceDetailPage({
  ...
  const [cars, allServices] = await Promise.all([publicFeaturedCars(3), getServices()]);
        ...
        aside={
          <div className="rounded-(--radius-card) bg-surface p-6">
            ...
            <a
              ...
              className="mt-2 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-field font-semibold transition-colors hover:bg-line"
            >
        ...
        <Shell>
          ...
          <Grid className="mt-8">
            {service.highlights.map((highlight, index) => (
              ...
              >
                <span className="font-display text-sm font-bold text-brand-bright">
                ...
                <ol className="mt-4 space-y-3">
                  {bookingSteps.map((step) => (
                    <li key={step.step} className="flex gap-3 text-[0.9375rem]">
                      <span className="font-display font-bold text-brand-bright">{step.step}</span>
        ...
        <Shell>
          ...
          <Grid className="mt-8">
            {others.map((item) => (
              ...
              >
                <span className="grid size-11 place-items-center rounded-(--radius-inner) bg-brand-tint text-brand-bright transition-colors group-hover:bg-brand group-hover:text-white">
