...
export async function generateMetadata({
  ...
  return {
    title: `${car.name} hire`,
    description: `${car.name} (${car.year}) for hire. ${car.tagline}. ${car.specs.seats} seats, ${car.specs.transmission}, ${car.specs.fuel}.`,
  };
}
...
export default async function CarDetailPage({
  ...
  const related = await getRelatedCars(slug, 3);
      ...
      <section className="bg-paper pt-[calc(4.75rem+var(--gap))] sm:pt-[calc(5.5rem+var(--gap))]">
        <Shell>
          <div className="rounded-(--radius-shell) bg-surface-alt px-6 py-10 sm:px-8 lg:px-12 lg:py-12">
            <Link
              href="/fleet"
              className="mb-7 inline-flex items-center gap-2 text-sm font-medium text-muted transition-colors hover:text-brand"
            >
